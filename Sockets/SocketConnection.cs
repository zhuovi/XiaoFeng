...
namespace XiaoFeng.Sockets
{
    ...
    public class SocketConnection
    {
        ...
        private string OSName
        {
            get
            {
                return OS.Platform.GetOSPlatform().ToString();
            }
        }
        /// <summary>
        /// 接收数据
        ...
        /// 连接委托
        /// </summary>
        public event NewConnectionEventHandler OnConnection;
        /// <summary>
        /// 消息委托
        /// </summary>
        ...
        /// 连接认证
        /// </summary>
        public Func<SocketConnection, bool> SocketAuth;
        /// <summary>
        /// 设置是Socket类型
        /// </summary>
        public Action<SocketConnection> SetSocketType { get; set; }
        #endregion
        ...
        private void ReceiveSocketData(IAsyncResult socket)
        {
            SocketConnection Conn = this;
            Socket SocketClient = Conn.ConnectionSocket;
            ...
            if (this.CancelToken.IsCancellationRequested)
            {
                ...
                lock (this.ConnectionSocket)
                {
                    ...
                    OnDisconnected?.Invoke(this.EndPoint, EventArgs.Empty);
                }
                return;
            }
            try
            {
                ...
                if (length == 0)
                {
                    if (!this.IsConnected(SocketClient))
                    {
                        lock (this.ConnectionSocket)
                        {
                            if (this.ConnectionSocket == null || this.ConnectionSocket.RemoteEndPoint == null) return;
                            /*断开连接*/
                            this.OnDisconnected?.Invoke(this.EndPoint, EventArgs.Empty);
                        }
                    }
                    return;
                }
                ...
                {
                    SocketClient.BeginReceive(buffer, 0, buffer.Length, SocketFlags.None, new AsyncCallback(ReceiveSocketData), Conn);
                }
                string msg;
                ...
                OnMessage?.Invoke(Conn, msg, EventArgs.Empty);
                if (OnMessageByte != null)
                {
                    byte[] msgBytes = new byte[length];
                    Buffer.BlockCopy(ReceivedDataBuffer, 0, msgBytes, 0, length);
                    OnMessageByte?.Invoke(Conn, msgBytes, EventArgs.Empty);
                }
            }
            catch (SocketException ex)
            {
                LogHelper.Error(ex, "接收信息失败[socket]");
                lock (Conn.ConnectionSocket)
                {
                    if (Conn.ConnectionSocket == null || Conn.ConnectionSocket.RemoteEndPoint == null) return;
                    /*断开连接*/
                    OnDisconnected?.Invoke(Conn.EndPoint, EventArgs.Empty);
                    /*出错消息*/
                    OnError?.Invoke(Conn.ConnectionSocket.RemoteEndPoint, ex);
                }
            }
        }
        #endregion
        ...
        private void ReceiveWebSocketData(IAsyncResult status)
        {
            ...
            catch (SocketException ex)
            {
                ...
                lock (this.ConnectionSocket)
                {
                    if (this.ConnectionSocket == null || this.ConnectionSocket.RemoteEndPoint == null) return;
                    OnDisconnected?.Invoke(this.EndPoint, EventArgs.Empty);
                    OnError?.Invoke(this.ConnectionSocket.RemoteEndPoint, ex);
                }
            }
        }
        #endregion
        ...
        public void ManageHandshake(IAsyncResult status)
        {
            ...
            RawClientHandshake = decoder.GetString(ReceivedDataBuffer, 0, HandshakeLength);
            this.RequestHeader = RawClientHandshake;
                ...
                if (this.SocketAuth != null)
                {
                    if (!this.SocketAuth(this))
                    {
                        if (this.OnDisconnected != null &&
                            ...
                            this.OnDisconnected(this.ConnectionSocket.RemoteEndPoint, EventArgs.Empty);
                        return;
                    }
                }
