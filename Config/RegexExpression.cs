...
namespace XiaoFeng.Config
{
    ...
    public class RegexExpression : ConfigSet<RegexExpression>
    {
        ...
        public string DateTime { get; set; } = @"^(((\d{2}|\d{4})(-|\/)(\d{1,2})\4(\d{1,2})(\s+|T)\d{1,2}\:\d{1,2}(\:\d{1,2}(\.\d+)?)?Z?)|((\d{2}|\d{4})年(\d{1,2})月(\d{1,2})日(\s*)\d{1,2}(时|点)\d{1,2}分(\d{1,2}秒)?))(\s*\+\d{2}:\d{2})?$";
        /// <summary>
        /// 日期或日期+时间正则
        /// </summary>
        [Description("日期或日期+时间正则")]
        public string DateOrTime { get; set; } = @"^((\d{2}|\d{4})(-|\/)(\d{1,2})\3(\d{1,2})|((\d{2}|\d{4})年(\d{1,2})月(\d{1,2})日))((\s*|T)((\d{1,2}\:\d{1,2}(\:\d{1,2}(\.\d+)?)?Z?)|(\d{1,2}(时|点)\d{1,2}分(\d{1,2}秒)?)))?(\s*\+\d{2}:\d{2})?$";
        /// <summary>
        /// IP正则
        /// </summary>
        [Description("IP正则")]
